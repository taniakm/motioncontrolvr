...
{
	//TRACE(_T("CSerialBuffer : (tid:%d) AddData(char) called "), GetCurrentThreadId ());	
...
{
	//TRACE (_T("CSerialBuffer : (tid:%d) AddData(%s,%d) called "), GetCurrentThreadId (),szData.c_str (),iLen);	
...
{
	//TRACE (_T("CSerialBuffer : (tid:%d) AddData(char*,%d) called "), GetCurrentThreadId (),iLen);	
	//ASSERT ( strData != NULL );
...
{
	//TRACE (_T("CSerialBuffer : (tid:%d) AddData(%s) called "), GetCurrentThreadId (),szData.c_str () );	
...
{
	//TRACE (_T("CSerialBuffer : (tid:%d) Read_N() called "), GetCurrentThreadId ());	
	...
	UnLockBuffer();
	//TRACE (_T("CSerialBuffer : (tid:%d) Read_N returned %d bytes (data:%s)  "), GetCurrentThreadId (),alTempCount,((szData)).c_str());	
	return alTempCount;
}
...
{
	//TRACE (_T("CSerialBuffer : (tid:%d) Read_Upto called "), GetCurrentThreadId ());
  ...
  //TRACE (_T("CSerialBuffer : (tid:%d) Read_Available returned (data:%s)  "), GetCurrentThreadId (),((szData)).c_str());	
...
bool CSerialBuffer::Read_Upto( std::string &szData,char chTerm,long  &alBytesRead ,HANDLE & hEventToReset)
{
	...
	UnLockBuffer();
  //TRACE (_T("CSerialBuffer : (tid:%d) Read_Upto returned %d bytes (data:%s)  "), GetCurrentThreadId (),alBytesRead,((szData)).c_str());	
	return abFound;
}
